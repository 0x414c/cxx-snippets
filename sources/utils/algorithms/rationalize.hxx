...
{
  /**
   * @brief Finds the  "best"  rational approximation  (p / q)  of the real number  `x'  that satisfies given
   *   restrictions  (error,  maximal denominator value  &  maximal iterations count)  by using continued fractions.
   * For the reference see  `http://mathworld.wolfram.com/ContinuedFraction.html'.
   * The absolute error value will be  |p / q - x|.
   * TODO:  Functions from  `cmath'  aren't constexpr by standard.
   * @tparam TIntegral
  ...
  )
  {
    ...
    ASSERT (! (rel_tol < TFloatingPoint (0)), "`rel_tol'  mustn't be less than  `0'");
    ASSERT (! (abs_tol < TFloatingPoint (0)), "`abs_tol'  mustn't be less than  `0'");
...
#pragma GCC diagnostic pop
     ...
     *   or the loop will stuck at division by zero later.
     */
    ...
    /*
     * NOTE:  Indices in variables' names are shifted by one.
     * We use only p[-1],  q[-1]  &  p[0],  q[0] as starting coefficients.
     * Rather than keeping entire sequence of p's  &  q's we'll keep and reuse coefficients computed on current
     *   iteration  (p[1]  &  q[1])  to compute new ones on the next iteration.
     */
    while (true)
    {
      ++ iteration;
